...
export const Contact = () => {
  ...
  const sendEmail = (e) => {
    e.preventDefault();
    let response =fetch("http://127.0.0.1:3000/contact", {
      method: "POST",
      headers: {
        "Content-Type": "application/json;charset=utf-8",
      },
    ...
    emailjs.sendForm('service_1kz7b78', 'template_i5aon3h', form.current, '1Fuugxzu_ptoota4O')
      .then((result) => {
      ...
      if (response.status == 200) {
        setStatus({ succes: true, message: 'Your response is been submitted'});
  
      } else {
        console.log(response.status);
        setStatus({ succes: false, message: 'Response Submitted'});
      }
  };
                ...
                <form ref={form} onSubmit={sendEmail}>
                  <Row>
                    ...
                    <Col size={12} className="px-1">
                      <textarea rows="6" name='message' value={formDetails.message} placeholder="Issue you are facing" onChange={(e) => onFormUpdate('message', e.target.value)}></textarea>
